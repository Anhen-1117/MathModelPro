...
/** 渲染后的 HTML 内容（marked v15 parse() 返回 Promise） */
...
async function renderContent(content: string) {
	...
	try {
		...
		renderedContent.value = content; // 解析失败时显示原文
	}
}
...
<template>
  <div :class="[
    ...
    <div class="flex flex-col gap-1 flex-1">
      ...
      <span v-else-if="props.type === 'agent' && props.agentType === AgentType.CODER"
        class="text-2xl select-none mb-1">👨‍💻</span>
      <span v-else-if="props.type === 'agent' && props.agentType === AgentType.WRITER"
